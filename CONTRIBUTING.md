# Knowledge Graph Specification Contribution Guide

This repository contains the specification text as well as Pull Requests with
suggested improvements and contributions.
...
becomes ever more important as the community broadens.
...
## Contributing to Knowledge Graph Libraries

A common point of confusion for those who wish to contribute to Knowledge Graph
is where to start. In fact, you may have found yourself here after attempting to
make an improvement to a Knowledge Graph library. Should a new addition be made
to the Knowledge Graph spec first or a Knowledge Graph library first?
Admittedly, this can become a bit of a
[chicken-or-egg](https://en.wikipedia.org/wiki/Chicken_or_the_egg) dilemma.

Knowledge Graph libraries seek to be "spec compliant", which means they
discourage changes that cause them to behave differently from the spec as
written. However, they also encourage pull requests for changes that accompany
an RFC _proposal_ or RFC _draft_. In fact, a spec contribution RFC won't be
_accepted_ until it has experience being implemented in a Knowledge Graph
library.
...
Suggested contributions should use these principles to guide the details of an
RFC and decisions to move forward.

- **Backwards compatibility**

  Once a graph is written, it should always mean the same thing. Future changes
  should not change the meaning of existing syntactic meaning or in any other
  way cause an existing compliant graph service to become non-compliant for
  prior versions of the spec.

- **Performance is a feature**

  Graph typically avoids syntax or behaviors that could jeopardize runtime
  efficiency, or that make demands of Graph services which cannot efficiently be
  fulfilled.

- **Favor no change**

  Proposed changes must meet a very high bar of added value. The burden of proof
  is on the contributor to illustrate this value.
...
- **Simplicity and consistency over expressiveness and terseness**
  ...
  absent from Graph. "Possible but awkward" is often favored over more complex
...
- **Understandability is just as important as correctness**
  ...
  The Knowledge Graph spec, despite describing technical behavior, is intended
  to be read by people. Use natural tone and include motivation and examples.

## RFC Contribution Champions

Contributing to spec requires a lot of dedicated work. To set clear expectations
and provide accountability, each proposed RFC (request for comments) must have a
_champion_ who is responsible for addressing feedback and completing next steps.
An RFC may have multiple _champions_. The spec editors are not responsible for
completing RFCs which lack a _champion_ (though an editor may be a _champion_
for an RFC).
...
stage at a time, but may advance multiple stages at a time.
...
Therefore, it is unwise to rely on a _proposal_ in a production Graph service.
Graph libraries _may_ implement _proposals_, though are encouraged to not enable
the _proposed_ feature without explicit opt-in.
...
implement the _draft_ in Graph libraries along with tests to gain confidence
...
production Graph Service. Graph libraries _should_ implement _drafts_ to provide
valuable feedback, though are encouraged not to enable the _draft_ feature
without explicit opt-in when possible.
...
to be deployed in Graph libraries.
...
An _accepted_ RFC is merged into the Graph spec's main branch by an editor and
